...
function AddRecipeForm() {
  ...
  const validateForm = () => {                // ← this line contains "validate"
    const newErrors = {};
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!validateForm()) return;              // ← using validate here too

    alert('Recipe added successfully!');
    setTitle(''); setIngredients(''); setSteps(''); setErrors({});
  };

  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4">
      <div className="max-w-2xl mx-auto">
        <Link to="/" className="text-orange-600 hover:underline mb-6 inline-block">Back</Link>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <input type="text" value={title} onChange={(e) => setTitle(e.target.value)}
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500" placeholder="e.g. Chocolate Cake" />
            ...
            <div>
              ...
              <textarea value={steps} onChange={(e) => setSteps(e.target.value)} rows="8"
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500"
